...
namespace ing::logging::attributes
{
    ...
    protected:
        ...
        using held = std::conditional_t<std::is_invocable_r_v<Val, Gen>, gen, val>;
        ...
        public:
            template<typename ...Args>
            explicit impl(Args&&... args) : held(std::forward<Args>(args)...) {}
    ...
    public:
        ...
        typename held::held& get() noexcept
        {
            return static_cast<impl*>(this->get_impl())->get();
        }

        typename held::held const& get() const noexcept
        {
            return static_cast<impl*>(this->get_impl())->get();
        }
        ...
        void set(Args&&... args)
        {
            get() = typename held::held(std::forward<Args>(args)...);
        }
    };
}
